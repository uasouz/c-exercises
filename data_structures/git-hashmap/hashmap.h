...
/*
 * struct git-hashmap is the hash table structure. Members can be used as follows,
...
struct hashmap {
    ...
    /* total number of entries (0 means the git-hashmap is empty) */
    unsigned int private_size; /* use hashmap_get_size() */

    /*
     * tablesize is the allocated size of the hash table. A non-0 value
     * indicates that the git-hashmap is initialized. It may also be useful
...
/* git-hashmap functions */

/*
 * Initializes a git-hashmap structure.
 *
 * `map` is the git-hashmap to initialize.
 *
...
/*
 * Frees a git-hashmap structure and allocated memory.
 *
...
/*
 * Returns the git-hashmap entry for the specified key, or NULL if not found.
 *
 * `map` is the git-hashmap structure.
 *
...
/*
 * Returns the git-hashmap entry for the specified hash code and key data,
...
/*
 * Returns the next equal git-hashmap entry, or NULL if not found. This can be
...
/*
 * Adds a git-hashmap entry. This allows to add duplicate entries (i.e.
...
/*
 * Adds or replaces a git-hashmap entry. If the git-hashmap contains duplicate
...
/*
 * Removes a git-hashmap entry matching the specified key. If the git-hashmap contains
...
/*
 * Used to iterate over all entries of a git-hashmap. Note that it is
 * not safe to add or remove entries to the git-hashmap while
 * iterating.
 */
struct hashmap_iter {
    struct hashmap *map;
 ...
 *
 * Normally, the git-hashmap keeps track of the number of items in the map
 ...
 * threaded callers (because the git-hashmap code does not know about the
 ...
 * duplicates.
 *
 * Uses a git-hashmap to store the pool of interned strings.
 */
